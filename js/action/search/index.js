 ...
 * @param pageSize
 * @param token 与该搜索关联的唯一token， 用于取消搜索
 * @param favoriteDao
 * @param popularKeys
 * @param callBack
 * @returns {function(*=)}
...
export function onSearch(inputKey, pageSize, token, favoriteDao, popularKeys, callBack) {
    return dispatch => {
        ...
        fetch(getFetchUrl(inputKey)).then(response => {//如果任务取消，则不做任何处理
            ...
            if (hasCancel(token, true)) {//如果任务取消，则不做任何处理并移除token
                console.log('user cancel');
                return;
            }
            if (!responseData || !responseData.items || responseData.items.length === 0) {// 无数据处理
