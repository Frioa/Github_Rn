...
import {Button, Linking, Clipboard, Text, View, TouchableOpacity} from 'react-native';
...
export default class AboutMePage extends Component<Props> {
    ...
    onClick(tab) {
       ...
       if (tab.url) {
        ...
        if (tab.account && tab.account.indexOf('@') > -1) { // 邮箱
        ...
        if (tab.account) {
            Clipboard.setString(tab.account); // 放入剪切板中
    ...
    _item(data, isShow, key){
        return ViewUtil.getSettingItem(()=>{
            this.setState({
                [key]: !this.state[key] // 根据Key取反展示或者取消Item
            })
