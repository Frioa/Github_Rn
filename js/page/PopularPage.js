...
import {FlatList, StyleSheet, TouchableOpacity,ActivityIndicator, View, RefreshControl, Text} from 'react-native';
    ...
    _genTabs() {
        ...
        keys.forEach((item, index) => {
            if (item.checked) { // 选中情况下
                tabs['tab' + index] = {
                    screen: props => <PopularTabPage {...props} tabLabel={item.name} theme = {theme}/>, // 点击Tab，传递属性（路由传递参数）
                    navigationOptions: {
                        title: item.name
                    }
                }
